...
```mermaid
graph LR
    ASR[Lightning ASR] -->|Exports| Metrics[/metrics Endpoint]
    SM[ServiceMonitor] -->|Discovers| Metrics
    Prom[Prometheus] -->|Scrapes| SM
    Prom -->|Stores| TSDB[(Time Series DB)]
    Adapter[Prometheus Adapter] -->|Queries| Prom
    HPA[HPA Controller] -->|Gets Metrics| Adapter
    HPA -->|Scales| ASR
